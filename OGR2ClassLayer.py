...
class OGR2LayersClassLayer:
	...
	if (self.providerName == "WFS"):
	    #if the input and output epsg have the same projection add WFS like OpenLayers.Layer.WFS
	    if self.inEpsg == self.outputEpsg:
		self.OpenLayersFormat = "WFS"
		self.outputFormat = "WFS"
		self.outputName = self.source
	    #else write shapefile from WFS and reproject it with the right projection and load it like OpenLayers.Layer.GML
	    else:
		nameFileWFS = os.path.abspath(os.path.join(self.pathSave,str(self.name) + "_temp.shp"))
		...
		writeShape = QgsVectorFileWriter.writeAsShapefile(self.layer, nameFileWFS, "UTF8",inputQgsReference)
		if writeShape == QgsVectorFileWriter.NoError:
		    OGR2ogr.Ogr2Ogr(nameFileWFS, str(self.destPathName), self.outputEpsg, self.inEpsg, self.outputFormat)
		    self.OpenLayersFormat = "GML"
		#else
		    #AGGIUNGERE ERRORE SE NON RIESCE A CONVERTIRE IL FILE
	#add other vector type
		...
		raise Exception, "There is a bug with OGR and Spatialite and now it isn't possible support Spatialite\n" # WORK
		#QMessageBox.warning(self.iface.mainWindow(), self.MSG_BOX_TITLE, ("There is a bug with OGR and Spatialite and now it isn't possible support Spatialite\n"), QMessageBox.Ok, QMessageBox.Ok)
		#return 0
	    ...
	    OGR2ogr.Ogr2Ogr(str(self.source), str(self.destPathName), self.outputEpsg, self.inEpsg, self.outputFormat)
	    self.OpenLayersFormat = "GML"
	...
	htmlLayer = ['var ' + self.name + ' = new OpenLayers.Layer.' + self.OpenLayersFormat + '("' + self.name + ' ' + self.outputFormat + '", "' + self.outputName + '"']
	...
	#outputFormat is GML
	    ...
	    if self.query == 'cluster':
		htmlLayer.append(', {strategies: [strategy]')
		#if qgis style it's choosen add it, I use this indentation for { because it is already open with strategy cluster
		if self.query == 'qgis':
		    htmlLayer.append(', styleMap: ' + self.name + '_style')
		htmlLayer.append('}')
